...
class DigitalRublePlatform:
    ...
    def _ensure_seed_state(self) -> None:
        # Инициализация только технических структур (генезис‑блок и т.п.).
        # Автоматическое создание банков, пользователей и транзакций отключено.
        return

    def reset_state(self) -> None:
        """Полный сброс имитационной модели для чистого старта.

        Удаляются все пользователи, банки, транзакции, смарт‑контракты,
    ...
    def create_banks(self, count: int) -> List[int]:
        bank_ids: List[int] = []
        # Чтобы избежать нарушения UNIQUE по имени банка, подбираем
        # следующий свободный числовой суффикс исходя из уже существующих имён.
        ...
        for idx in range(count):
            ...
            # Локальная база данных конкретного банка (узла ФО)
            # для моделирования распределённого хранения.
            from database import DatabaseManager  # локальный импорт во избежание циклов
    ...
    def get_transactions(self, tx_type: Optional[str] = None) -> List[Dict]:
        if tx_type:
            rows = self.db.execute(
                "SELECT * FROM transactions WHERE tx_type = ? ORDER BY timestamp DESC",
                (tx_type,),
                fetchall=True,
            )
        else:
    ...
    def exchange_to_digital(self, user_id: int, amount: float) -> None:
        ...
        if user["fiat_balance"] < amount:
            raise ValueError("Недостаточно средств на безналичном счете")
        self.db.execute(
            """
            UPDATE users
            SET fiat_balance = fiat_balance - ?, digital_balance = digital_balance + ?
            WHERE id = ?
            """,
    ...
    def fund_offline_wallet(self, user_id: int, amount: float) -> None:
        ...
        if user["digital_balance"] < amount:
            raise ValueError("Недостаточно средств на цифровом кошельке")
        self.db.execute(
            """
            UPDATE users
            SET digital_balance = digital_balance - ?, offline_balance = offline_balance + ?
            WHERE id = ?
            """,
            (amount, amount, user_id),
        )
    ...
    # region --- Transaction lifecycles -------------------------------------------
    def create_online_transaction(
        self, sender_id: int, receiver_id: int, amount: float, channel: str
    ) -> Dict:
        if amount <= 0:
            raise ValueError("Сумма должна быть положительной")
        ...
        if sender["digital_balance"] < amount:
            self._log_activity(
                actor=sender["name"],
                stage="Валидация ФО",
        ...
        self._apply_balances(sender_id, receiver_id, amount, mode="digital")
        return tx
    ...
    def create_offline_transaction(
        ...
        if sender["offline_balance"] < amount:
            raise ValueError("Недостаточно средств в оффлайн кошельке")
        context = TransactionContext(
            sender_id=sender_id,
            receiver_id=receiver_id,
            amount=amount,
    ...
    def sync_offline_transactions(self) -> Dict[str, int]:
        ...
        for row in rows:
            ...
            if sender["digital_balance"] + sender["offline_balance"] < row["amount"]:
                conflicts += 1
                self.db.execute(
                    "UPDATE offline_transactions SET status = 'КОНФЛИКТ', conflict_reason = ? WHERE id = ?",
                    ("Обнаружено двойное расходование", row["offline_id"]),
                )
                ...
                self._log_activity(
                    ...
                    details=f"Транзакция {row['id']} отклонена из-за двойной траты",
                    context="Оффлайн",
                )
                continue
            processed += 1
            self._apply_balances(
                row["sender_id"],
                row["receiver_id"],
                row["amount"],
                mode="digital",
    ...
    def _execute_contract(self, contract) -> None:
        ...
        if creator["digital_balance"] < contract["amount"]:
            raise ValueError(
                f"Недостаточно средств для исполнения контракта {contract['id']}"
            )
        context = TransactionContext(
    ...
    def _log_online_transaction(self, sender: Dict, receiver: Dict, amount: float) -> None:
        ...
        steps = [
            ...
            ("Менеджер активностей", "Шаг 9. Фиксация времени отправки в ЦБ"),
            ...
            ("Менеджер активностей", "Шаг 15. Фиксация времени подтверждения"),
            ...
            ("Менеджер активностей", "Шаг 20. Фиксация завершения обработки"),
        ]
    ...
    def _log_block_flow(self, block, context: TransactionContext) -> None:
        ...
        steps = [
            ...
            ("Сборщик метрик", "Фаза 6. Шаг 20. Фиксация времени формирования блока"),
        ]
        for actor, stage in steps:
